...
const inputVariants = cva(
  "flex w-full rounded-md border bg-transparent px-3 py-2 text-sm transition-colors file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-content-tertiary focus-ring disabled:cursor-not-allowed disabled:opacity-50",
  {
    variants: {
      variant: {
  ...
  return (
    <div class="w-full">
      ...
      <input
        ...
        aria-describedby={
          local.error || local.helperText ? `${inputId}-description` : undefined
        }
        {...others}
      />
      {(local.error || local.helperText) && (
        <p
          ...
          class={`mt-1 text-sm ${
            local.error ? "text-danger-500" : "text-content-secondary"
          }`}
        >
          {local.error || local.helperText}
        </p>
