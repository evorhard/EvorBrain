...
export const activeGoals = createMemo(() =>
  state.items.filter(goal => !goal.archived_at)
);

export const completedGoals = createMemo(() =>
  state.items.filter(goal => goal.completed_at)
);
...
export const goalActions = {
  ...
  async fetchAll() {
    ...
    try {
      ...
      const message = error instanceof EvorBrainError 
        ? error.message 
